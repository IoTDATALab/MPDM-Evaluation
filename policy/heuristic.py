...
@attrs
class HeuristicAlgorithm(BasePolicy):
    s = attrib(type=int)
    parts = attrib(type=int, default=10)
    parallelization = attrib(type=bool, default=False)
    accuracy_type = attrib(type=str, default="Mean")

    def __attrs_post_init__(self):
        super(HeuristicAlgorithm, self).__attrs_post_init__()
        ...
        if self.parts > 1:
            ...
            self.bw_ranges = [tuple([min_bandwidth, max_bandwidth])]
    ...
    def evaluate_objective(
        ...
        accuracy_func = temp_system.median_accuracy if self.accuracy_type == "Median" else temp_system.average_accuracy
        ...
        if stream is None:
            ...
            return wchebyshev([1, 1, 1], [
                round(
                    self.normalize(accuracy_func(), min_accuracy,
                                   max_accuracy), 3),
                round(
                    self.normalize(len(temp_system.served_streams()),
                                   min_number, max_number), 3),
                round(
                    self.normalize(temp_system.bandwidth_reserved(),
                                   bw_range[0], bw_range[1])
                    if len(self.system.paradigms) > 1 else self.normalize(
                        temp_system.bandwidth_reserved(), min_bandwidth,
                        max_bandwidth), 3)
            ], weight)
        else:
            temp_total_number = len(temp_system.served_streams()) + 1
            temp_accuracy = accuracy_func(
                extended_accuracies=[decision.accuracy])
            ...
            return wchebyshev(
                ...
                [
                    round(
                        self.normalize(temp_accuracy, min_accuracy,
                                       max_accuracy), 3),
                    round(self.normalize(temp_total_number, min_number,max_number),
                          3),
                    round(
                        self.normalize(temp_bandwidth_reserved, bw_range[0],
                                       bw_range[1])
                        if len(self.system.paradigms) >1 else
                        self.normalize(temp_bandwidth_reserved, min_bandwidth,
                                       max_bandwidth), 3)
                ],
                weight)
    ...
    def solve_details(self):
        if self.parallelization:
            ...
            results = []
            for weight in tqdm(self.weights, desc="Heuristic"):
        ...
        points = list(
            map(
                lambda system: {
                    ...
                    "median_accuracy": system.median_accuracy(),
        ...
        if self.accuracy_type == "Median":
            accuracy_string = "median_accuracy"
        else:
            accuracy_string = "average_accuracy"
        indexes = list(
            map(
                lambda _: all([
                    _[accuracy_string] in self.system.accuracy, _['served'] in
                    self.system.number, _['bandwidth_reserved'
        ...
        if len(points) > 0:
            ...
            mask = paretoset(dataset[[
                accuracy_string, 'served_ratio', 'bandwidth_reserved_ratio'
            ]],
                             sense=["max", "max", "max"])
